...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            mouth_x = int((left_lip.x + right_lip.x) / 2 * w)
            mouth_y = int((left_lip.y + right_lip.y) / 2 * h)

            
            mouth_open_val = abs(upper_lip.y - lower_lip.y) * h
            open_mouth = mouth_open_val > 15
            ...
            if food_visible:
                ...
                if mouth_x and mouth_y and open_mouth:
    ...
    if lives <= 0:
        cv2.putText(frame, "Game Over!", (200, 220), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 160, 255), 3)
        cv2.imshow("Food Eater(press 'q' to exit)", frame)
        cv2.waitKey(5000)
        break
